...
- **On an update/land refusal, branch on `blockers[].code`, not the prose** (codes are stable, `reasons` wording is not): `stale_lockfiles` = generated churn — `glittering get`, commit, re-run · `submodule_ahead`/`submodule_dirty`/pin codes — run the blocker's `hint` · `user_changes` — genuinely the user's work, stop and ask
...
- **test/analyze**: `{ packages: [{ path, status, details_file, ... }], summary }` — read `details_file` for details. Test statuses: `pass`/`fail`/`error`/`timeout` (analyze has no `timeout`). `timeout` = the package hit the per-package cap and was killed: counts are partial, and its `details_file` carries `failures` plus `incomplete` — tests that started but never finished, i.e. the hang suspects. `summary.success` is the single verdict: false when anything failed, errored **or timed out** — a timed-out run is not green, whatever the pass counts say.
- **stats**: `{ threshold, packages: [{ path, source_files, source_lines, test_files, test_lines, oversized_count, details_file }], summary }`
- **git**: `{ repo: { branch, dirty, ahead_remote, upstream, upstream_configured, ... }, submodules: [{ ..., ahead_parent, behind_parent, nested_drift }] }` — `upstream` may be an `origin/<branch>` fallback; `upstream_configured: false` means no tracking is set. `nested_drift` lists nested submodules off their pin (otherwise only top-level submodules are inspected)
- **git check**: `{ clean: bool, issues: [{ repo, severity, type, message, fix }], summary }` — `type`: `dirty`/`detached`/`unpushed`/`stash`/`no_upstream`/`ahead_parent`/`behind_parent`/`nested_drift`; `fix` is a runnable command where one exists
...
- **worktree update**: `{ worktree, path, branch, base_branch, success, base: { action, from_ref, to_ref, new_commits, submodules, blockers }, merge: { status, ref, commits_integrated, conflicts }, submodules: [git sync results, spanning the pre-flight heal and the post-merge convergence], reasons, blockers: [{ repo, code, message, paths, hint }], warnings, hint }` — `base.action`: `up_to_date`/`fast_forwarded`/`skipped_dirty`/`missing`/`failed` · `merge.status`: `merged`/`up_to_date`/`conflicts`/`skipped`/`failed`. `reasons` non-empty = refused, no merge (pins may have been healed — see `submodules`); every refusal has a `blockers` entry. Codes: `merge_in_progress`/`user_changes`/`stale_lockfiles`/`submodule_dirty`/`submodule_ahead`/`submodule_diverged`/`submodule_unsynced`/`status_unreadable`
- **worktree land**: `{ worktree, branch, base_branch, landed, success, reasons, blockers, submodules: [the feature worktree's pre-flight heal], pushed/skipped/failed: [{ path, status, ref, error }], base: { action, from_ref, to_ref, new_commits, pushed, submodules }, warnings, hint }` — `pushed` is in publish order (submodules first, then `.` = the feature branch); `landed` = the base branch moved, `success` = that plus everything pushed. `blockers` adds land-specific codes: `not_landable`/`not_contained`/`pin_rewind`/`remote_diverged`/`no_upstream`/`base_not_ready`
