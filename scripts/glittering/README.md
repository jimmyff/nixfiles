...
Test packages report `pass`/`fail`/`error`/`timeout`. A `timeout` means the per-package cap was hit and the run was killed: counts are partial, and the detail file's `incomplete` list names tests that started but never finished (the hang suspects). `summary.success` is the single verdict, and a live `glittering test` exits `1` exactly when it is false.
