...
#
# Each wrapper uses mkSopsWrapper (from sops-wrappers-lib.nix) to provide:
#   - Shared preamble: set -euo pipefail, SOPS/VAULT vars, 3-tier age key
#     resolution (SOPS_AGE_KEY > SOPS_AGE_KEY_FILE > ~/.config/sops/age/keys.txt),
#     require_file() helper
#   - Standard template (minisign, apple-notarize): tempdir, binary decrypt,
#     env export, sops exec-env, child command
#   - Body override (android-sign, apple-sign): preamble only, caller manages
#     mode dispatch, tempdir, and decrypt logic
#
