...
- **The kickoff prompt is generated, not stored.** Compose it at session start from the tracker's header prose (the workstream-invariant constraints, stated once per tracker) plus the row's brief. The brief stores only what generation cannot recover. `docket start <tracker> [<id>]` performs exactly that composition and returns the prompt.
